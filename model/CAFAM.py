...
class LargeKernelSpatialAttn(nn.Module):
    def __init__(self, dim, groups=8):
        ...
        self.conv0 = nn.Conv2d(dim, dim, 5, padding=2, groups=dim, bias=False)
        self.conv_spatial = nn.Conv2d(dim, dim, 7, stride=1, padding=9, groups=dim, dilation=3, bias=False)
        self.conv1 = nn.Conv2d(dim, dim, 1, bias=False, groups=groups)
        self.norm = nn.BatchNorm2d(dim)
        self.cross_fuse = nn.Conv2d(dim * 2, dim, 1, bias=False, groups=groups)
...
class CAFA(nn.Module):
    def __init__(self, in_channels=1024, num_heads=4, decode_channels=96, decode_channels1=32):
        ...
        self.groups_proj = 8
        self.qkv = nn.Conv2d(in_channels, in_channels, 1, bias=False, groups=self.groups_proj)

        self.spatial_cross_attn = LargeKernelSpatialAttn(in_channels, groups=self.groups_proj)

        self.output_proj = nn.Conv2d(in_channels, in_channels, 1, bias=False, groups=self.groups_proj)

        self.weight_spatial = nn.Parameter(torch.zeros(1))

        hidden_dim = in_channels // 2  # 512
        self.ffn = nn.Sequential(
            nn.Conv2d(in_channels, hidden_dim, 1, bias=False),
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(inplace=True),
            nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, groups=hidden_dim, bias=False),  # depthwise
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(inplace=True),
        ...
        self.conv1 = BasicConv(128, decode_channels, kernel_size=3, dilation=1, padding=1, groups=decode_channels1, bias=False, relu=False)
        self.conv2 = BasicConv(256, decode_channels, kernel_size=3, dilation=1, padding=1, groups=decode_channels1, bias=False, relu=False)
        self.conv3 = BasicConv(512, decode_channels, kernel_size=3, dilation=1, padding=1, groups=decode_channels1, bias=False, relu=False)
        self.conv4 = BasicConv(1024, decode_channels, kernel_size=3, dilation=1, padding=1, groups=decode_channels1, bias=False, relu=False)
    ...
    def forward(self, res1, res2, res3, res4, tes1, tes2, tes3, tes4):
        ...
        Q = self.qkv(res4).view(B, self.num_heads, C // self.num_heads, H * W).permute(0, 1, 3, 2)
        K = self.qkv(tes4).view(B, self.num_heads, C // self.num_heads, H * W).permute(0, 1, 3, 2)
        V = self.qkv(tes4).view(B, self.num_heads, C // self.num_heads, H * W).permute(0, 1, 3, 2)

        attn_scores = torch.matmul(Q, K.transpose(-2, -1)) / ((C // self.num_heads) ** 0.5)
        attn_weights = F.softmax(attn_scores, dim=-1)
        global_out = torch.matmul(attn_weights, V)
        global_out = global_out.permute(0, 1, 3, 2).contiguous().view(B, C, H, W)
        global_out = self.output_proj(global_out)
        ...
        res4_e = res4 * outRGBT
        tes4_e = tes4 * outRGBT
        res3_e = res3 * self.conv_sem3(self.up2(outRGBT))
        tes3_e = tes3 * self.conv_sem3(self.up2(outRGBT))
        res2_e = res2 * self.conv_sem2(self.up4(outRGBT))
        tes2_e = tes2 * self.conv_sem2(self.up4(outRGBT))

        res1 = self.conv1(res1)
        tes1 = self.conv1(tes1)
        res2_e = self.conv2(res2_e)
        tes2_e = self.conv2(tes2_e)
        res3_e = self.conv3(res3_e)
        tes3_e = self.conv3(tes3_e)
        res4_e = self.conv4(res4_e)
        tes4_e = self.conv4(tes4_e)

        res2_e = F.interpolate(res2_e, size=(res1h, res1w), mode='bicubic', align_corners=False)
        res3_e = F.interpolate(res3_e, size=(res1h, res1w), mode='bicubic', align_corners=False)
        res4_e = F.interpolate(res4_e, size=(res1h, res1w), mode='bicubic', align_corners=False)
        mid_res = torch.cat([res2_e, res3_e, res4_e], dim=1)

        tes2_e = F.interpolate(tes2_e, size=(res1h, res1w), mode='bicubic', align_corners=False)
        tes3_e = F.interpolate(tes3_e, size=(res1h, res1w), mode='bicubic', align_corners=False)
        tes4_e = F.interpolate(tes4_e, size=(res1h, res1w), mode='bicubic', align_corners=False)
        mid_tes = torch.cat([tes2_e, tes3_e, tes4_e], dim=1)
